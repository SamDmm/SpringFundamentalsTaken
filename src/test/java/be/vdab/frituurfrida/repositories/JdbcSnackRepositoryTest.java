...
public class JdbcSnackRepositoryTest extends AbstractTransactionalJUnit4SpringContextTests {
	...
	private long idVanTestSnack() {
		return super.jdbcTemplate.queryForObject("select id from snack where naam='test'", Long.class);
	}
	@Test
	public void update() {
		...
		assertEquals(0, BigDecimal.ONE.compareTo(super.jdbcTemplate.queryForObject("select prijs from snacks where id=?", BigDecimal.class, id)));
	}
	...
	public void findByBeginNaam() {
		...
		long aantalSnacks = super.jdbcTemplate.queryForObject("select count(*) from snacks where naam like 't%'", Long.class);
